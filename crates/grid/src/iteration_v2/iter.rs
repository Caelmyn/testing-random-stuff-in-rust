...
impl<'a, T: 'a> Iter<'a, T> {
    ...
    #[inline(always)]
    unsafe fn internal_next_unchecked(&mut self, offset: isize) -> *const T {
        if std::mem::size_of::<T>() == 0 {
            self.end = (self.end as *const u8).wrapping_offset(-offset) as *const T;
            self.ptr.as_ptr()
        } else {
...
impl<'a, T> Iterator for Iter<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        unsafe { Some(&*self.internal_next_unchecked(offset as isize)) }
    }
...
mod tests {
    ...
    fn iter() {
        ...
        let iter = Iter::new(&vec, WIDTH, area);
        iter.for_each(|item| println!("{}", item));

        let iter = Iter::new(&vec, WIDTH, Area::new(8, 8, 9, 9));
        iter.for_each(|item| println!("{}", item));
    }
}
