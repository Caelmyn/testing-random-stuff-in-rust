...
fn test_iter_mut() {
    ...
    grid.iter_mut().enumerate().for_each(|(index, val)| *val = index as i32);
...
fn test_iter_mut_over() {
    ...
    assert_eq!(grid.iter_mut().count(), 100);
    assert_eq!(grid.iter().sum::<i32>(), 0);
    grid.iter_over_mut(Area::new(0, 0, 10, 10)).for_each(|val| *val = 10);
    assert_eq!(grid.iter().sum::<i32>(), 1000);
    ...
    grid.iter_mut().enumerate().for_each(|(index, val)| val.id = index);
    ...
    grid.iter_over_mut(Area::new(959, 489, 964, 759)).enumerate().for_each(|(_, val)| val.set_status());
    for index in 0..1000*1000 {
        ...
        if is_contained_in_area(coords, Area::new(959, 489, 964, 759)) {
            assert_eq!(val, &Witness { id: index, status: true });
        } else {
            assert_eq!(val, &Witness { id: index, status: false });
        }
    }

    grid.iter_mut().enumerate().for_each(|(_, val)| val.unset_status());
    grid.iter_over_mut(Area::new(0, 0, 0, 999)).enumerate().for_each(|(_, val)| val.set_status());
    for index in 0..1000*1000 {
        let coords = coord_from_idx(index, 1000);
        let val = grid.get(coords.0, coords.1).unwrap();
